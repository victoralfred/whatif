"""Smoke tests for `whatif.types.primitives`.

These are Phase 1.1 tests. Per phases.md:
- Each type constructs correctly.
- The public surface (`DecimalString`, `JsonPrimitive`) is what `whatif.types`
  re-exports.
- Import budget: `whatif.types.primitives` should import in < 50ms.
...
from whatif.types import DecimalString, JsonPrimitive
from whatif.types import primitives as primitives_module
...
class TestImportBudget:
    """Phase 1 gate: `import whatif.types` must be cheap.
    ...
    def test_reload_is_cheap(self) -> None:
        ...
        importlib.import_module("whatif.types.primitives")

...
| `ci_computable=False` on a required cohort cannot Ship | Per V0_1_DECISION_RECORD §2's CI split: `ci_computable: bool` is the structural fact (was bootstrap successful?) read by `ci_availability_guard` which emits `ci_unavailable_for_required_cohort` at `blocks_all` severity for any required cohort with `ci_computable=False`. `compute_verdict` routes `blocks_all` to `Inconclusive` regardless of policy. There is no `--accept-no-ci` escape hatch (§6); `policy.max_ci_width` is the lever for accepting wider but computable CIs and operates on the policy side via the deferred `ci_meaningful` guard (Phase 3, cascade-tracked). Tests: `tests/unit/whatif/decision/guards/test_ci_availability.py` covers the emission + boundary behavior. |
| Floor vs policy concerns are partitioned on `CohortResult` | Per V0_1_DECISION_RECORD §2: `ci_computable` (structural, `blocks_all`) and `ci_meaningful` (policy-quality, `blocks_ship`, deferred Phase 3) are separate fields. `CohortResult.__post_init__` raises `InvariantViolationError` if `ci_meaningful=False` is constructed without `ci_computable=True` — meaningfulness is the quality assessment of a CI that exists; an incoherent combination cannot be constructed. Tests: `TestCiMeaningfulSplit` in `tests/unit/whatif/types/test_cohort.py` pins all four valid/invalid shapes; `TestCiMeaningfulBoundary` in `test_ci_availability.py` pins that `ci_availability_guard` reads only `ci_computable`. |
